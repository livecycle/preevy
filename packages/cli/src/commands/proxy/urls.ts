...
import { tunnelServerFlags, urlFlags, utils } from '@preevy/cli-common'
...
export default class Urls extends ProfileCommand<typeof Urls> {
  ...
  async run(): Promise<unknown> {
    ...
    const commandArgs = [`--id=${envId}`, ...utils.formatFlagsToArgs(flags, PreevyUrlsCmd.flags), ...Object.values(pick(this.args, Object.keys(PreevyUrlsCmd.args))).map(x => `${x}`)]
