...
import { DriverName, LocationType, machineDrivers } from '../../lib/machine'
...
export default class Init extends BaseCommand {
  ...
  async run(): Promise<unknown> {
    ...
    if (this.flags.from) {
      ...
      const { locationType } = await inquirer.prompt<{ locationType: LocationType }>([
        {
          type: 'list',
          name: 'locationType',
      ...
      if (locationType === 's3') {
        const {
          region,
          bucket,
        } = await inquirer.prompt<{ region: string; bucket: string }>([
          {
            type: 'list',
            name: 'region',
          ...
          },
          {
            ...
            default: async (
              ...
              return accountId ? s3DefaultBucketName({
                profileAlias,
                accountId,
              }) : undefined
            },
        ...
        const {
          project,
          bucket,
        } = await inquirer.prompt<{ project: string; bucket: string }>([
          {
            type: 'input',
            name: 'project',
          ...
          },
          {
            ...
            default: (
              ...
              profileAlias,
              project: answers.project as string,
            }),
          },
        ])
      ...
      await this.config.runCommand('profile:create', ['--log-level', this.flags['log-level'] ?? 'info', profileAlias, location, '--driver', driver, ...Object.entries(driverFlags)
        .map(([key, value]) => `--${driver}-${key}=${value}`)])
    }
