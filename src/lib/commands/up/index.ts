...
const scriptExecuter = ({ sshClient, log } :{
  ...
  await sshClient.putFiles(scripts.DIR, [script], destination)
  try {
    log.debug('env', opts.env)
    return sshClient.execCommand(`env; chmod +x ${script} && ./${script}`, { cwd: destination, env: opts.env })
  } finally {
...
const up = async ({
  ...
  try {
    ...
    if (!installed) {
      for (const script of scripts.FROM_BARE) {
        log.debug(`executing machine script`, script)
    ...
    log.debug('tunnel opts', tunnelOpts)

    let [tunnelHost, tunnelPort] = tunnelOpts.hostname.split(':')
    ...
    for (const script of scripts.INSTANCE_SPECIFIC) {
      log.debug(`executing instance-specific script`, script)
      await execScript(script, { env })
    }

    log.debug('testing tunnel')
    const { stdout } = await sshClient.execCommand('sudo preview_tunnel_meta')
    const { id } = JSON.parse(stdout)
    log.debug(`tunnel id: ${id}`)
  } finally {
    sshClient.dispose()
  }
  
  return { machine, keyPair }
}
